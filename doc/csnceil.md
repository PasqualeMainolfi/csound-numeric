...
`csnceil` replaces every element by the smallest integer not less than it, so
`-1.2` becomes `-1` and `0.5` becomes `1`. It is the mirror of
...
```csound
handle:CsnArr = csnceil(source:CsnArr)
handle:CsnArr = csnceil(source:CsnArr, trig:k)
```
