...
Elementwise less than, as a 0/1 array.

## Description

`csnlt` compares every element against a scalar and returns an array of the same
...
```csound
handle:CsnArr = csnlt(source:CsnArr, value:i)
handle:CsnArr = csnlt(source:CsnArr, value:k)
handle:CsnArr = csnlt(source:CsnArr, value:k, trig:k)
```
