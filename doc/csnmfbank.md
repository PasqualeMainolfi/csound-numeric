...
Build a triangular mel filterbank matrix in linear scale.

## Description

`csnmfbank` returns the filterbank itself rather than applying it: a real
...
Weights run from zero at a band's edges to one at its centre, or to the
Slaney scale factor when normalization is on.
...
```csound
bank:CsnArr = csnmfbank(nfft:i, bands:i, low_freq:i, high_freq:i, sample_rate:i, slaney_norm:i)
```
...
instr 1
    ...
    bank:CsnArr = csnmfbank(32, 4, 0, 24000, 48000, 0)
