...
Compute the DCT-II of a real array along one axis.

## Description

`csndcttwo1d` applies the discrete cosine transform of type II
along one axis of a real array, matching SciPy's definition with `norm=None`:

```
y[k] = 2 sum(n=0..N-1) x[n] cos(pi k (2n+1) / (2N))
```

The output has the same shape as the input. The transform is carried by a real
FFT over a symmetric extension of length `2 * N`, so the cost grows like a
...
```csound
coeffs:CsnArr = csndcttwo1d(source:CsnArr)
coeffs:CsnArr = csndcttwo1d(source:CsnArr, axis:i)
coeffs:CsnArr = csndcttwo1d(source:CsnArr, trig:k)
coeffs:CsnArr = csndcttwo1d(source:CsnArr, trig:k, axis:i)
```
...
instr 1
    ...
    coeffs:CsnArr = csndcttwo1d(source)
    csnprint coeffs
    turnoff
endin
</CsInstruments>
