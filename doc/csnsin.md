...
`csnsin` returns the sine of every element, the argument in **radians**. Use
...
```csound
handle:CsnArr = csnsin(source:CsnArr)
handle:CsnArr = csnsin(source:CsnArr, trig:k)
```
