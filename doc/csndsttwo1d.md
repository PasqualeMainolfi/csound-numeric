...
Compute the DST-II of a real array along one axis.

## Description

`csndsttwo1d` applies the discrete sine transform of type II
along one axis of a real array, matching SciPy's definition with `norm=None`:

```
y[k] = 2 sum(n=0..N-1) x[n] sin(pi (k+1) (2n+1) / (2N))
```
...
```csound
coeffs:CsnArr = csndsttwo1d(source:CsnArr)
coeffs:CsnArr = csndsttwo1d(source:CsnArr, axis:i)
coeffs:CsnArr = csndsttwo1d(source:CsnArr, trig:k)
coeffs:CsnArr = csndsttwo1d(source:CsnArr, trig:k, axis:i)
```
...
instr 1
    ...
    coeffs:CsnArr = csndsttwo1d(source)
    csnprint coeffs
    turnoff
endin
</CsInstruments>
