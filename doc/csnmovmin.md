...
Moving minimum over a window.

## Description

`csnmovmin` replaces every element by the smallest of the elements around it,
within a window of `winsize` — a running floor.

Paired with [csnmovmax](csnmovmax.md) it brackets a signal: the two together give
...
```csound
handle:CsnArr = csnmovmin(source:CsnArr, winsize:i)
handle:CsnArr = csnmovmin(source:CsnArr, winsize:i, axis:i)
handle:CsnArr = csnmovmin(source:CsnArr, winsize:k, axis:k)
handle:CsnArr = csnmovmin(source:CsnArr, winsize:k, axis:k, trig:k)
csnmovmin(source:CsnArr, winsize:i)
csnmovmin(source:CsnArr, winsize:i, axis:i)
csnmovmin(source:CsnArr, winsize:k, axis:k)
csnmovmin(source:CsnArr, winsize:k, axis:k, trig:k)
```
...
; csnmovmin.csd
;
; A running floor. With csnmovmax it brackets the signal, and the difference of
; the two is the local range.
; -----------------------------------------------------------------------------
...
instr 1
    ...
    smoothed:CsnArr = csnmovmin(data, 3)
