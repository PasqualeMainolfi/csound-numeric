...
`csncos` returns the cosine of every element, the argument in **radians**. Use
[csndegtorad](csndegtorad.md) when the data is in degrees.
...
```csound
handle:CsnArr = csncos(source:CsnArr)
handle:CsnArr = csncos(source:CsnArr, trig:k)
```
