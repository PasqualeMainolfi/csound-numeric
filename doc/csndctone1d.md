...
Compute the DCT-I of a real array along one axis.

## Description

`csndctone1d` applies the discrete cosine transform of type I
along one axis of a real array, matching SciPy's definition with `norm=None`:

```
y[k] = x[0] + (-1)^k x[N-1] + 2 sum(n=1..N-2) x[n] cos(pi k n / (N-1))
```
...
way; a length whose factorization is ragged only costs more time. The shortest input is two elements, since the extension of a
single sample is empty.
...
```csound
coeffs:CsnArr = csndctone1d(source:CsnArr)
coeffs:CsnArr = csndctone1d(source:CsnArr, axis:i)
coeffs:CsnArr = csndctone1d(source:CsnArr, trig:k)
coeffs:CsnArr = csndctone1d(source:CsnArr, trig:k, axis:i)
```
...
instr 1
    ...
    coeffs:CsnArr = csndctone1d(source)
    csnprint coeffs
    turnoff
endin
</CsInstruments>
