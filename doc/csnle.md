...
Elementwise less than or equal, as a 0/1 array.

## Description

`csnle` compares every element against a scalar and returns an array of the same
...
```csound
handle:CsnArr = csnle(source:CsnArr, value:i)
handle:CsnArr = csnle(source:CsnArr, value:k)
handle:CsnArr = csnle(source:CsnArr, value:k, trig:k)
```
