...
Arccosine of every element.

## Description

`csnacos` returns the arccosine of every element, in radians, in the range
`[0, pi]`.
...
```csound
handle:CsnArr = csnacos(source:CsnArr)
handle:CsnArr = csnacos(source:CsnArr, trig:k)
```
