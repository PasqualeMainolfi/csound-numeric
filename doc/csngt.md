...
Elementwise greater than, as a 0/1 array.

## Description

`csngt` compares every element against a scalar and returns an array of the same
...
```csound
handle:CsnArr = csngt(source:CsnArr, value:i)
handle:CsnArr = csngt(source:CsnArr, value:k)
handle:CsnArr = csngt(source:CsnArr, value:k, trig:k)
```
