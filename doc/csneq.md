...
Elementwise equality, as a 0/1 array.

## Description

`csneq` compares every element against a scalar and returns an array of the same
...
Real only. A NaN is not equal to anything, itself included, so it never appears in this
mask; [csnargisnan](csnargisnan.md) is the way to find one.

## Syntax

```csound
handle:CsnArr = csneq(source:CsnArr, value:i)
handle:CsnArr = csneq(source:CsnArr, value:k)
handle:CsnArr = csneq(source:CsnArr, value:k, trig:k)
```
