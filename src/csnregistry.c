...
int32_t update_slot_array_locked(
    ...
    if (slot->rt_locked) {
        *err = "[csnarray] Array is on a real-time path and cannot be reallocated at perf time (clear the mark with csnrtunlock, or pass irt=0 at the audio source it descends from)";
        return NOTOK;
    }
