...
instr 2
    ...
    iMedRows:CsnArr = csnmedfilt1d(iMovMat, 3, 1)
    ...
    csnmedfilt1d(iMedRowsIn, 3, 1)
    iMedRowsInValues[][] = csntoarray(iMedRowsIn)
    assert(iMedRowsInValues[0][2] == 4 && iMedRowsInValues[1][0] == 4)
...
instr 4
    ...
    iDotValues[][] = csntoarray(iDot)
    iInnerValues[][] = csntoarray(iInner)
    assert(iDotValues[0][0] == 7 && iDotValues[1][1] == 22 && iInnerValues[0][1] == 11)
    ...
    iBatchAFlat:CsnArr = csnfromarray(array(1, 0, 0, 1, 2, 0, 0, 2))
    iBatchBFlat:CsnArr = csnfromarray(array(1, 2, 3, 4))
    iBatchAShape[] = fillarray(2, 2, 2)
    iBatchBShape[] = fillarray(1, 2, 2)
    iBatchA:CsnArr = csnreshape(iBatchAFlat, iBatchAShape)
    iBatchB:CsnArr = csnreshape(iBatchBFlat, iBatchBShape)
    iBatchProduct:CsnArr = csnmatmul(iBatchA, iBatchB)
    iBatchProductValues[][][] = csntoarray(iBatchProduct)
    assert(iBatchProductValues[0][1][0] == 3 && iBatchProductValues[1][1][1] == 8)
    iBatchNorm:CsnArr = csnnorm(iBatchA, 2, 1)
    iBatchMedian:CsnArr = csnmedian(iBatchA, 1)
    iBatchQuantile:CsnArr = csnquantile(iBatchA, 0.5, 1)
    iBatchArgMax:CsnArr = csnargmax(iBatchA, 1)
    iBatchNormValues[][] = csntoarray(iBatchNorm)
    iBatchMedianValues[][] = csntoarray(iBatchMedian)
    iBatchQuantileValues[][] = csntoarray(iBatchQuantile)
    iBatchArgMaxValues[][] = csntoarray(iBatchArgMax)
    assert(iBatchNormValues[1][0] == 2 && iBatchMedianValues[0][1] == 0.5)
    assert(iBatchQuantileValues[1][1] == 1 && iBatchArgMaxValues[3][0] == 1 && iBatchArgMaxValues[3][1] == 1 && iBatchArgMaxValues[3][2] == 1)
    iEmptyBatch:CsnArr = csnempty(array(0, 2, 2))
    iEmptyBatchProduct:CsnArr = csnmatmul(iEmptyBatch, iBatchB)
    iEmptyBatchShape[] = csnshape(iEmptyBatchProduct)
    assert(csnsize(iEmptyBatchProduct) == 0 && iEmptyBatchShape[0] == 0)
    iReservedBatch:CsnArr = csnempty(array(1, 2, 2))
    iReservedBatchProduct:CsnArr = csnmatmul(iReservedBatch, iBatchB)
    assert(csnsize(iReservedBatchProduct) == 0)
